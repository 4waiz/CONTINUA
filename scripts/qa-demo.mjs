...
// A card that flashes past in a third of a second is unreadable even though it
// is technically two lines. This caught a "seven." orphan left by a greedy
// split of the longest sentence in the script.
const orphans = times.filter((cue) => cue.to - cue.from < 0.8);
check(
  'no subtitle flashes past too fast to read',
  orphans.length === 0,
  orphans.length
    ? orphans.map((cue) => `${cue.from.toFixed(1)}s (${(cue.to - cue.from).toFixed(2)}s)`).join(', ')
    : `shortest ${Math.min(...times.map((cue) => cue.to - cue.from)).toFixed(2)} s`,
);
...
const lines = [
  ...
  '',
  // A list, not trailing-space line breaks: the checkpoint gate warns on
  // trailing whitespace, and it is right to.
